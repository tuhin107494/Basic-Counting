...
ll catalan(ll n)
{
    ...
    ll res=(fac[2*n]);
    ll temp=fac[n+1]*fac[n];
    temp=ext_euclid(mod,temp);
    res=(res*temp)%mod;

    ll smlprime=smallprime[2*n]-smallprime[n+1]-smallprime[n];
    ll lrgprime=largeprime[2*n]-largeprime[n+1]-largeprime[n];

    res=(res*big_mod(2,smlprime,mod))%mod;
    res=(res*big_mod(5,lrgprime,mod))%mod;
